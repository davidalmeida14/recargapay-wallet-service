...
class TransferControllerE2ETest extends EndToEndTest {
    ...
    class Transfer {
        ...
        @Test
        @DisplayName("prevents duplicate transfers with different idempotency keys when balance is insufficient")
        void preventsDoubleSpendingWithInsufficientBalance() throws Exception {
            ...
            try {
                ...
                assert successCount == 1 : "Expected exactly one successful transfer, got " + successCount + " (status1=" + status1 + ", status2=" + status2 + ")";
                assert failureCount == 1 : "Expected exactly one failed transfer, got " + failureCount + " (status1=" + status1 + ", status2=" + status2 + ")";
            } finally {
